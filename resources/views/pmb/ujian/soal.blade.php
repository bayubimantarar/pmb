...
function startExam(){
    ...
    '<input type="hidden" class="kodeSoal" value="{{ $kodeSoal }}" required />' +
    '</div>' +
    '</form>',
    buttons: {
        formSubmit: {
            ...
            action: function () {
                ...
                var kodeSoal = this.$content.find('.kodeSoal').val();
                if(!token){
                    $.alert('Token perlu diisi');
                    return false;
                }
                $.ajax({
                    url : '/pmb/ujian/soal/cek-token/'+token,
                    type: 'get',
                            ...
                            $.confirm({
                                ...
                                buttons: {
                                    mulai: {
                                        btnClass: 'btn-blue',
                                        action: function(){
                                            window.location.replace('/pmb/ujian/soal/mulai/'+kodeSoal+'/'+token);
                                        },
                                    },
                                    batal: {
                                        action: function(){
